...
张静是 `计算机图形学` 和 `计算机视觉` 领域的专家。他可以熟练使用多种编程语言，例如 `C`、`C++`、`Python`、`JavaScript` 和 `glsl` 等，并已于 Windows、Linux、Android、iOS、Web 等平台发布过产品。

张静的职业生涯围绕着 `GPU`，从高层的游戏引擎和 GPU 加速的数据可视化应用，到中层的图形 API 工具，再到底层的 GPU 性能优化都有专业的项目经历。
...
为心动的第一方和第二方项目提供可重用的工具链，提升产品质量和研发效率：

- 负责一款 UE4 MMORPG 项目的渲染、客户端性能及图形工具开发，提供端到端的性能监控及定位服务。
- 开发[渲染医生](https://github.com/xindong/render-doctor) 和 [包体医生](https://github.com/xindong/pkg-doctor) 项目，为自研团队定期输出性能优化及包体优化报告。
- 对心动投资的公司进行性能优化及技术培训，服务中的项目包括《香肠派对》、《不休的乌拉拉》、《江南百景图》等。
- 从零开始搭建心动与硬件厂商的技术合作，包括各大手机厂商与 GPU 供应商，满足项目组对特定硬件的优化需求。
